...
import { FaPlus, FaMinus, FaCube, FaBorderStyle, FaLock, FaLockOpen, FaUndo, FaRedo, FaExpand, FaTrash, FaCircle, FaSquare, FaMountain, FaDrawPolygon, FaCubes, FaSeedling } from "react-icons/fa";
...
const ToolBar = ({ terrainBuilderRef, mode, handleModeChange, axisLockEnabled, setAxisLockEnabled, placementSize, setPlacementSize, setGridSize, undoRedoManager, currentBlockType, environmentBuilderRef }) => {
	...
	const handleExportMap = async () => {
		try {
			// First ensure we have the latest terrain data from the database
			const dbTerrainData = await DatabaseManager.getData(STORES.TERRAIN, "current");
			
			// If we found data in the database but not in the TerrainBuilder, sync it
			if (dbTerrainData && Object.keys(dbTerrainData).length > 0 && 
				(!terrainBuilderRef.current.getCurrentTerrainData() || 
				 Object.keys(terrainBuilderRef.current.getCurrentTerrainData()).length === 0)) {
				 
				console.log("Syncing terrain data from database before export...");
			...
			<div className="controls-container">
				...
				<div className="control-group">
					...
					<div className="control-button-wrapper">
						...
						<Tooltip text="Seed Generator">
							<button
								onClick={() => handleToolToggle("seedGenerator")}
								className={`control-button ${activeTool === "seedGenerator" ? "selected" : ""}`}>
								<FaSeedling />
							</button>
						</Tooltip>
					</div>
